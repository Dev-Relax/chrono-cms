import type {
  AuthUser, AdminUser, MediaFile, Page, PageTranslation, Post, PostTranslation, Comment,
  PaginatedResponse, SiteSettings, ThemeConfig, BrandConfig, NavConfig, CommentStatus,
} from "../types/index.js";
...
const postsQueryToRecord = ({ strict, ...rest }: PostsQueryParams): Record<string, string | number | undefined> => ({
  ...rest,
  ...(strict ? { strict: "1" } : {}),
});
...
export const postsApi = {
  ...
  search: (q: string, lang?: string) =>
    request<{ data: Post[] }>(`/posts/search?q=${encodeURIComponent(q)}${lang ? `&lang=${lang}` : ""}`),
  ...
  create: (payload: PostPayload) =>
    request<{ data: AdminPost }>("/admin/posts", { method: "POST", body: payload }),

  update: (id: string, payload: Partial<PostPayload>) =>
    request<{ data: AdminPost }>(`/admin/posts/${id}`, { method: "PUT", body: payload }),
...
export type PageTranslationPayload = TranslationPayload & {
  heroContent?: { subtitle?: string; image?: string; ctaText?: string; ctaUrl?: string } | null;
};
...
export const pagesApi = {
  ...
  adminList: () =>
    request<{ data: Page[] }>("/admin/pages"),

  adminGet: (id: string) =>
    request<{ data: AdminPage }>(`/admin/pages/${id}`),

  create: (payload: PagePayload) =>
    request<{ data: AdminPage }>("/admin/pages", { method: "POST", body: payload }),

  update: (id: string, payload: Partial<PagePayload>) =>
    request<{ data: AdminPage }>(`/admin/pages/${id}`, { method: "PUT", body: payload }),
...
export const usersApi = {
  ...
  create: (payload: UserPayload) =>
    request<{ data: AdminUser }>("/admin/users", { method: "POST", body: payload }),

  update: (id: string, payload: UserUpdatePayload) =>
    request<{ data: AdminUser }>(`/admin/users/${id}`, { method: "PUT", body: payload }),
...
export const settingsApi = {
  ...
  update: (payload: { themeConfig?: ThemeConfig; brandConfig?: BrandConfig; navConfig?: NavConfig }) =>
    request<{ data: SiteSettings }>("/admin/settings", { method: "PUT", body: payload }),
};
...
export const resolveMediaUrl = (path: string): string => {
  ...
  const origin = explicitApi
    ? explicitApi.replace(/\/api\/?$/, "")
...
export const mediaApi = {
  list: () =>
    request<{ data: MediaFile[] }>("/admin/media"),
  ...
  delete: (filename: string) =>
    request<void>(`/admin/media/${encodeURIComponent(filename)}`, { method: "DELETE" }),
};
...
export const webhooksApi = {
  ...
  create: (payload: WebhookPayload) =>
    request<{ data: unknown }>("/admin/webhooks", { method: "POST", body: payload }),

  update: (id: string, payload: Partial<WebhookPayload>) =>
    request<{ data: unknown }>(`/admin/webhooks/${id}`, { method: "PUT", body: payload }),

  delete: (id: string) =>
    request<void>(`/admin/webhooks/${id}`, { method: "DELETE" }),

  test: (id: string) =>
    request<{ ok: boolean; status: number }>(`/admin/webhooks/${id}/test`, { method: "POST" }),
};

export const apiKeysApi = {
  list: () =>
    request<{ data: unknown[] }>("/admin/apikeys"),

  create: (name: string) =>
    request<{ data: { id: string; name: string; prefix: string; createdAt: string; key: string } }>(
      "/admin/apikeys", { method: "POST", body: { name } }
    ),

  delete: (id: string) =>
    request<void>(`/admin/apikeys/${id}`, { method: "DELETE" }),
};

export const bulkApi = {
  posts: (action: "publish" | "unpublish" | "delete", ids: string[]) =>
    request<{ affected: number }>("/admin/posts/bulk", { method: "POST", body: { action, ids } }),
};

export const activityApi = {
  list: () =>
    request<{ data: unknown[] }>("/admin/activity"),
};
...
export interface CmsStats {
  ...
  recentPosts: {
    ...
    featured: boolean; publishedAt: string | null; updatedAt: string;
...
export const commentsApi = {
  ...
  listForPost: (postId: string) =>
    request<{ data: Comment[] }>(`/posts/${postId}/comments`),

  /** Public: submit a new comment */
  submit: (postId: string, payload: CommentPayload) =>
    request<{ data: Comment; message: string }>(
  ...
  adminList: (params: CommentsQueryParams = {}) =>
    request<PaginatedResponse<Comment>>(
      `/admin/comments${buildQuery({ ...params })}`
    ),

  /** Admin: pending badge count */
  pendingCount: () =>
    request<{ count: number }>("/admin/comments/pending-count"),
...
export const previewApi = {
  ...
  create: (postId: string) =>
    request<{ data: { token: string; expiresIn: number } }>(
  ...
  get: (token: string, lang?: string) =>
    request<{ data: Post; preview: boolean }>(
      `/preview/${token}${lang ? `?lang=${lang}` : ""}`
    ),
};
...
export const revisionsApi = {
  ...
  restore: (postId: string, revId: string) =>
    request<{ data: AdminPost }>(`/admin/posts/${postId}/revisions/${revId}/restore`, { method: "POST" }),
};
