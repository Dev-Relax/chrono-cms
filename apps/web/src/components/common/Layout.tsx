...
export const NavTransitionContext = React.createContext<React.TransitionStartFunction>(
  (fn) => fn(),
);
...
export const AdminSidebar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetch = () => {
      commentsApi.pendingCount().then(({ count }) => setPendingCount(count)).catch(() => {/* ignore */});
    };
    fetch();
  ...
  return (
    <aside className="flex h-screen w-[220px] shrink-0 flex-col border-r border-slate-800 bg-slate-950">
      <div className="flex h-14 items-center border-b border-slate-800 px-4">
        <Link to="/" className="flex items-center gap-2 font-semibold hover:opacity-90 transition-opacity">
                  ...
                  <li key={item.to}>
                    ...
                    >
                      ...
                      {item.to === "/admin/comments" && pendingCount > 0 && (
                        <span className="ml-auto rounded-full bg-yellow-500 px-1.5 py-0.5
        ...
        {user && (
          <div className="mb-2 flex items-center gap-2 px-3">
            ...
            <div className="min-w-0">
              <p className="truncate text-xs font-medium text-slate-300">{user.name ?? user.email}</p>
...
export const Layout: React.FC<Props> = ({ children, admin = false }) => {
  ...
  const textColor = isBold ? "text-white/90 hover:text-white" : "text-slate-400 hover:text-slate-100";
  ...
  const logo = (
    <Link to="/" className="flex items-center gap-2 font-semibold hover:opacity-90 transition-opacity">
      {savedBrand.logoUrl ? (
        <img src={savedBrand.logoUrl} alt={savedBrand.siteName} className="h-8 w-auto object-contain" />
      ) : (
        <>
          {isDefaultBrand && <span style={{ color: logoColor }} className="text-lg">⏱</span>}
          <span className="text-slate-100" style={{ color: logoColor }}>{savedBrand.siteName}</span>
        </>
      )}
    </Link>
  );
  ...
  const navLink = (to: string, label: React.ReactNode, key: string) => (
    <Link
      key={key}
      to={to}
      onClick={(e) => { e.preventDefault(); React.startTransition(() => navigate(to)); }}
  ...
  const nav = (
    <nav className="flex items-center gap-4 text-sm">
      {savedNav.items.filter((item) => !item.hidden).map((item) => {
        if (item.type === "blog") {
        ...
        if (item.type === "custom" && item.url) {
          return (
            <a key={item.id} href={item.url} target={item.url.startsWith("http") ? "_blank" : undefined}
               rel="noopener noreferrer" className={`transition-colors ${textColor}`}>
              {item.label}
            </a>
          );
        }
      ...
      {state.status === "authenticated" ? (
        <>
          ...
          <button onClick={() => { logout(); navigate("/login"); }} className={`transition-colors ${textColor}`}>
            Logout
          </button>
        </>
      ) : (
