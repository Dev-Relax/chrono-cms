...
const STYLES: Record<CalloutType, { bg: string; border: string; icon: string; label: string; iconColor: string }> = {
  info:    { bg: "bg-blue-950/40",    border: "border-blue-500",   icon: "ℹ",  iconColor: "text-blue-400",   label: "Info"    },
  warning: { bg: "bg-yellow-950/40",  border: "border-yellow-500", icon: "⚠",  iconColor: "text-yellow-400", label: "Warning" },
  danger:  { bg: "bg-red-950/40",     border: "border-red-500",    icon: "✕",  iconColor: "text-red-400",    label: "Danger"  },
  tip:     { bg: "bg-emerald-950/40", border: "border-emerald-500",icon: "✓",  iconColor: "text-emerald-400",label: "Tip"     },
};
      ...
      <div className={`relative rounded-lg border-l-4 px-4 pt-3 pb-3 my-4 ${bg} ${border}`}>
        ...
        {selected && (
          <div
            ...
            className="absolute -top-3 right-2 flex gap-1 z-10"
          >
            {(Object.keys(STYLES) as CalloutType[]).map((t) => (
              <button
                key={t}
                type="button"
                onMouseDown={(e) => { e.preventDefault(); updateAttributes({ calloutType: t }); }}
                className={[
        ...
        <div className="flex gap-2.5">
          <span contentEditable={false} className={`text-base shrink-0 mt-0.5 select-none ${iconColor}`}>
            {icon}
          </span>
