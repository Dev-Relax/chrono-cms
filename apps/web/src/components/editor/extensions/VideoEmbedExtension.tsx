...
const VideoEmbedView: React.FC<NodeViewProps> = ({ node, updateAttributes, deleteNode, selected }) => {
          ...
          <div className="flex gap-2">
            <input
              ...
              onKeyDown={(e) => {
                ...
                if (e.key === "Escape") { if (src) setEditing(false); else deleteNode(); }
              }}
            ...
            <button
              type="button"
              onMouseDown={(e) => { e.preventDefault(); if (src) setEditing(false); else deleteNode(); }}
      ...
      >
        ...
        {selected && (
          <div contentEditable={false} className="absolute top-2 right-2 flex gap-1.5">
            <button
              type="button"
              onMouseDown={(e) => { e.preventDefault(); setInput(src); setEditing(true); }}
...
export const VideoEmbedExtension = Node.create({
  ...
  renderHTML({ node, HTMLAttributes }) {
    return [
      "div",
      mergeAttributes(HTMLAttributes, { "data-video-embed": node.attrs["src"] as string }),
    ];
  },
