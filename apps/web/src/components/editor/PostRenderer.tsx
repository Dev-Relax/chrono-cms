...
const CALLOUT_STYLES: Record<CalloutType, { container: string; icon: string; iconColor: string }> =
  {
    ...
    tip: {
      container: "border-emerald-500 bg-emerald-950/30",
      icon: "✓",
      iconColor: "#34d399",
    },
  }
...
const renderNode = (node: TipTapNode, index: number): React.ReactNode => {
  ...
  switch (node.type) {
    ...
    case "image": {
      ...
      const img = <img src={src} alt={alt ?? ""} title={title} style={imgStyle} loading="lazy" />
    ...
    case "callout": {
      const calloutType = ((node.attrs?.["calloutType"] as string | undefined) ??
        "info") as CalloutType
      const styles = CALLOUT_STYLES[calloutType] ?? CALLOUT_STYLES.info
      return (
        <div
          key={key}
          className={`flex gap-2.5 rounded-lg border-l-4 px-4 py-3 my-4 ${styles.container}`}
        >
          <span
