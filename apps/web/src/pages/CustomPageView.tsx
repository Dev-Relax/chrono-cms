...
const HeroSection: React.FC<{ title: string; hero: HeroContent; cfg: PageConfig }> = ({ title, hero, cfg }) => {
  ...
  return (
    ...
    >
      <div className={`mx-auto ${layoutMaxWidth[cfg.layout ?? "default"]}`}>
        ...
        {hero.subtitle && (
          <p className="mt-4 text-lg text-slate-300 max-w-2xl">{hero.subtitle}</p>
        )}
        {hero.ctaText && (
          <a
            href={hero.ctaUrl ?? "#"}
...
const LangSwitcher: React.FC<{ page: Page; onNavigate: (slug: string) => void }> = ({ page, onNavigate }) => {
  ...
  return (
    <div ref={ref} className="relative shrink-0">
      ...
      {open && (
        <div className="absolute right-0 top-full z-30 mt-2 min-w-[140px]
          ...
          {page.hreflang?.map(({ locale, slug: localeSlug }) => {
            ...
            return (
              <button
                key={locale}
                onClick={() => { setOpen(false); if (!isActive) onNavigate(localeSlug); }}
                className={[
...
const CustomPageView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      document.title = prev;
      descEl?.remove(); ogTitle.remove(); ogDesc?.remove(); ogImg?.remove();
    };
  }, [page]);
    ...
    <Layout>
        ...
        <div className="mx-auto max-w-3xl space-y-6 animate-pulse">
          ...
          <div className="space-y-3">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className={`h-4 rounded bg-slate-800 ${i % 3 === 2 ? "w-2/3" : "w-full"}`} />
            ))}
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="rounded-lg bg-red-900/30 border border-red-800 px-4 py-3 text-red-400">
          ...
          <Link
            to="/"
            onClick={(e) => { e.preventDefault(); React.startTransition(() => navigate("/")); }}
        ...
        >
          ...
          <div className={`mx-auto px-4 py-8 ${maxW}`}>
            <div className="mb-6 flex items-center justify-between gap-4">
              ...
              <LangSwitcher page={page} onNavigate={(s) => React.startTransition(() => navigate(`/${s}`))} />
            </div>
