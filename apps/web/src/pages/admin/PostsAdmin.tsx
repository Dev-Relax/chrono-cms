...
const PostsAdmin: React.FC = () => {
  ...
  const fetchPosts = useCallback(() => {
    setLoading(true);
    postsApi
      .adminList({ limit: 100, ...(statusFilter ? { status: statusFilter } : {}) })
      .then(({ data }) => { setPosts(data); setSelected(new Set()); })
    ...
    <Layout admin>
      ...
      {selected.size > 0 && (
        <div className="mb-4 flex items-center gap-3 rounded-xl border border-brand-700 bg-brand-900/20 px-4 py-3">
          ...
          <div className="ml-auto flex flex-wrap gap-2">
            <button onClick={() => void doBulk("publish")} disabled={bulking}
            ...
            <button onClick={() => void doBulk("unpublish")} disabled={bulking}
            ...
            <button onClick={() => void doBulk("delete")} disabled={bulking}
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-800 py-20 text-center">
          <p className="text-slate-500">No posts{statusFilter ? ` with status "${statusFilter}"` : ""} yet.</p>
          <Link to="/admin/posts/new" className="mt-3 inline-block text-sm text-brand-400 hover:underline">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-900">
                ...
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Author</th>
              ...
              {posts.map((post) => (
                <tr key={post.id}
                  ...
                  <td className="px-4 py-3 max-w-xs">
                    ...
                    <span className="font-mono text-xs text-slate-600 truncate block">/{post.slug}</span>
                  </td>
...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => (
  <span className={[
    ...
    status === "PUBLISHED" ? "bg-emerald-900/40 text-emerald-400" : "bg-amber-900/30 text-amber-500",
  ].join(" ")}>
