...
const AdminDashboard: React.FC = () => {
  ...
  const fetchActivity = useCallback(() => {
    if (role !== "ADMIN") { setActLoading(false); return; }
    setActLoading(true);
    activityApi.list()
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/60 overflow-hidden">
              <div className="flex items-center justify-between border-b border-slate-800 px-5 py-3">
                <h2 className="text-xs font-semibold uppercase tracking-wider text-slate-500">Recent posts</h2>
                <Link to="/admin/posts" className="text-xs text-brand-400 hover:underline">View all →</Link>
              </div>
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/60 overflow-hidden">
              <div className="flex items-center justify-between border-b border-slate-800 px-5 py-3">
                <h2 className="text-xs font-semibold uppercase tracking-wider text-slate-500">Recent pages</h2>
                <Link to="/admin/pages" className="text-xs text-brand-400 hover:underline">View all →</Link>
              </div>
            ...
            {role === "ADMIN" && (
              <div className="rounded-xl border border-slate-800 bg-slate-900/60 overflow-hidden">
                <div className="border-b border-slate-800 px-5 py-3">
                  <h2 className="text-xs font-semibold uppercase tracking-wider text-slate-500">Activity log</h2>
                </div>
                {actLoading ? (
                  ...
                  <ul className="max-h-96 overflow-y-auto divide-y divide-slate-800/60">
                    {activity.map((entry) => (
                      <li key={entry.id}
                        ...
                        <span className="mt-0.5 shrink-0 text-sm">{ActionIcon[entry.action] ?? "•"}</span>
                        <div className="min-w-0">
                          <p className={`text-xs font-medium ${actionColor(entry.action)}`}>{entry.action}</p>
                          {entry.entityTitle && (
