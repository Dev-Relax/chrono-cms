...
const EMPTY_FORM = { name: "", url: "", secret: "", events: [] as string[], active: true };
...
const WebhooksAdmin: React.FC = () => {
  ...
  const openEdit   = (h: Webhook) => {
    setEditing(h);
    setForm({ name: h.name, url: h.url, secret: h.secret ?? "", events: h.events, active: h.active });
    setShowForm(true);
    setError(null);
  };
  ...
  const handleTest = async (h: Webhook) => {
    ...
    try {
      ...
      setTestStatus((s) => ({ ...s, [h.id]: res.ok ? `✓ ${res.status}` : `✗ ${res.status}` }));
    } catch (err) {
      setTestStatus((s) => ({ ...s, [h.id]: `✗ ${(err as Error).message}` }));
    }
    setTimeout(() => setTestStatus((s) => { const n = { ...s }; delete n[h.id]; return n; }), 4000);
  };
    ...
    <Layout admin>
          ...
          <form onSubmit={(e) => { void handleSave(e); }} className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
                  required placeholder="My webhook" className={inputCls} />
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium uppercase tracking-wider text-slate-500">URL</label>
                <input value={form.url} onChange={(e) => setForm((f) => ({ ...f, url: e.target.value }))}
                  required type="url" placeholder="https://example.com/hooks/cms" className={inputCls} />
              </div>
            </div>

            <div>
              <label className="mb-1 block text-xs font-medium uppercase tracking-wider text-slate-500">
                Signing Secret <span className="normal-case font-normal text-slate-600">(optional — sent as X-Webhook-Signature)</span>
              </label>
              <input value={form.secret} onChange={(e) => setForm((f) => ({ ...f, secret: e.target.value }))}
                placeholder="Leave blank to skip signing" className={inputCls} />
            </div>

            <div>
              <label className="mb-2 block text-xs font-medium uppercase tracking-wider text-slate-500">
                Events <span className="normal-case font-normal text-slate-600">(none selected = all events)</span>
              </label>
            ...
            <div className="flex items-center gap-3">
              <button type="button" onClick={() => setForm((f) => ({ ...f, active: !f.active }))}
                ...
                <span className={`inline-block h-4 w-4 rounded-full bg-white shadow transform transition-transform ${form.active ? "translate-x-4" : "translate-x-0"}`} />
              </button>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-800 py-20 text-center">
          ...
          <p className="mt-1 text-xs text-slate-600">Create one to start receiving event notifications.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {hooks.map((h) => (
            <div key={h.id} className="rounded-xl border border-slate-800 bg-slate-900/60 px-5 py-4">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-slate-200">{h.name}</span>
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${h.active ? "bg-emerald-900/40 text-emerald-400" : "bg-slate-800 text-slate-500"}`}>
                      {h.active ? "active" : "inactive"}
                    </span>
                    {testStatus[h.id] && (
                      <span className={`text-xs font-mono ${testStatus[h.id]?.startsWith("✓") ? "text-emerald-400" : "text-red-400"}`}>
                        {testStatus[h.id]}
                      </span>
                    )}
                  </div>
                  ...
                  {h.events.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {h.events.map((ev) => (
                        <span key={ev} className="rounded-full border border-slate-700 px-2 py-0.5 text-xs text-slate-500">{ev}</span>
                      ))}
                    </div>
                  )}
                  {h.events.length === 0 && (
                ...
                <div className="flex shrink-0 items-center gap-2">
                  <button onClick={() => void handleTest(h)}
                  ...
                  <button onClick={() => void handleDelete(h)}
