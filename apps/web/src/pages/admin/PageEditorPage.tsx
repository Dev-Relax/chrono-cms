...
import type { PageStatus, TipTapDoc, PageConfig, HeroContent, PageLayout } from "../../types/index.js";
...
const emptyLocale = (): LocaleData => ({
  ...
  heroSubtitle: "", heroImage: "", heroCtaText: "", heroCtaUrl: "",
});
...
const PageEditorPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    pagesApi.adminGet(id)
      .then(({ data }) => {
        ...
        if (data.pageConfig) {
          setPageConfig({ ...DEFAULT_PAGE_CONFIG, ...(data.pageConfig as Partial<PageConfig>) });
        }
        const map: LocaleMap = {};
  ...
  const commitAddLocale = (code: string) => {
    ...
    if (!normalized || locales[normalized]) {
      setAddingLocale(false); setNewLocaleInput(""); return;
    }
    setLocales((prev) => ({ ...prev, [normalized]: emptyLocale() }));
    setActiveLocale(normalized);
    setAddingLocale(false); setNewLocaleInput("");
    setSaved(false);
  };
  ...
  const removeLocale = (locale: string) => {
    ...
    setLocales((prev) => { const next = { ...prev }; delete next[locale]; return next; });
  ...
  const save = async (overrideStatus?: PageStatus) => {
    ...
    if (!defaultData?.title.trim()) { setError("Title is required"); return; }
    ...
    const payload = { defaultLocale, translations, status: overrideStatus ?? status, pageConfig };
    ...
    <Layout admin>
      ...
      <div className="mb-4 rounded-xl border border-slate-800 bg-slate-900/60 p-4 space-y-4">
        <div className="flex flex-wrap items-end gap-6">
          <div>
            <label className="block mb-1 text-xs font-medium uppercase tracking-wider text-slate-500">Default locale</label>
            <select value={defaultLocale} onChange={(e) => { setDefaultLocale(e.target.value); setSaved(false); }}
              ...
              {activeLocaleKeys.map((l) => (
                <option key={l} value={l}>{getLocaleFlag(l)} {l.toUpperCase()}</option>
              ))}
            </select>
          </div>

          <div>
            <label className="block mb-1 text-xs font-medium uppercase tracking-wider text-slate-500">Layout</label>
            <div className="flex rounded-lg border border-slate-700 overflow-hidden text-xs">
              {(["default", "wide", "full-width"] as PageLayout[]).map((lay) => (
                <button key={lay} type="button"
                  onClick={() => { setPageConfig((p) => ({ ...p, layout: lay })); setSaved(false); }}
                  className={[
                    "px-3 py-1.5 capitalize font-medium transition-colors",
                    pageConfig.layout === lay ? "bg-brand-600 text-white" : "text-slate-400 hover:text-slate-200",
                  ].join(" ")}
                >{lay}</button>
              ))}
            </div>
          ...
          <div className="flex items-center gap-4">
            <label className="flex items-center gap-2 cursor-pointer select-none text-sm text-slate-400">
              <input type="checkbox" checked={pageConfig.showToc}
                onChange={(e) => { setPageConfig((p) => ({ ...p, showToc: e.target.checked })); setSaved(false); }}
            ...
            <label className="flex items-center gap-2 cursor-pointer select-none text-sm text-slate-400">
              <input type="checkbox" checked={pageConfig.showHero}
                onChange={(e) => { setPageConfig((p) => ({ ...p, showHero: e.target.checked })); setSaved(false); }}
                className="h-4 w-4 rounded border-slate-600 bg-slate-800 accent-brand-500"
              />
              Hero section
            </label>
      ...
      <div className="mb-0 flex items-end gap-1 flex-wrap">
        {activeLocaleKeys.map((locale) => {
          ...
          return (
            ...
            >
              ...
              {isDefault && (
                <span title="Default locale"
                  className={["text-[9px] leading-none", isActive ? "text-brand-400" : "text-slate-600"].join(" ")}>★</span>
              )}
              {!hasFilled && (
                <span title="Translation missing"
                  className={["h-1.5 w-1.5 rounded-full", isActive ? "bg-amber-400" : "bg-amber-600"].join(" ")} />
              )}
              {canRemove && (
                <span role="button" title={`Remove ${locale.toUpperCase()} translation`}
        ...
        {addingLocale ? (
          <div className="relative flex items-end gap-1 rounded-t-lg border-l border-r border-t
            ...
            <input ref={addInputRef} value={newLocaleInput}
            ...
            />
            <button onClick={() => commitAddLocale(newLocaleInput)}
              className="text-[10px] text-brand-400 hover:text-brand-300">✓</button>
            <button onClick={() => { setAddingLocale(false); setNewLocaleInput(""); }}
            ...
            {suggestions.length > 0 && (
              <div className="absolute left-0 top-full z-20 mt-1 flex flex-wrap gap-1
                ...
                {suggestions.slice(0, 8).map((s) => (
                  <button key={s} onClick={() => commitAddLocale(s)}
          ...
          <button onClick={() => setAddingLocale(true)} title="Add a translation"
      ...
      <div className="mb-4 rounded-b-xl rounded-tr-xl border border-slate-800 bg-slate-900/60 p-5">
        {!hasContent && activeLocale !== defaultLocale && (
          <div className="mb-4 flex items-center gap-2 rounded-lg border border-amber-800/50
            ...
            <span>⚠</span>
            <span>Translation missing — fill in the <span className="font-mono font-bold">{defaultLocale.toUpperCase()}</span> tab first.</span>
          </div>
        )}

        <div className="mb-3 grid gap-3 sm:grid-cols-2">
          <div>
            <label className="block mb-1 text-xs font-medium uppercase tracking-wider text-slate-500">Title *</label>
            <input value={current.title} onChange={(e) => handleTitleChange(e.target.value)}
              placeholder="Page title" className={inputCls} />
          </div>
          <div>
            <label className="block mb-1 text-xs font-medium uppercase tracking-wider text-slate-500">Slug</label>
            <input value={current.slug} onChange={(e) => handleSlugChange(e.target.value)}
              placeholder="page-slug" className={inputCls + " font-mono text-slate-400"} />
          </div>
        </div>
          ...
          <div className="mt-3">
            <Collapsible title="Hero section">
              <div className="grid gap-3 md:grid-cols-2">
                <div>
                  <label className="block mb-1 text-xs font-medium uppercase tracking-wider text-slate-500">Subtitle / tagline</label>
                  <input value={current.heroSubtitle}
                    onChange={(e) => { setCurrentField("heroSubtitle", e.target.value); }}
                    placeholder="Empowering developers worldwide" className={inputCls} />
                </div>
                <div>
                  <label className="block mb-1 text-xs font-medium uppercase tracking-wider text-slate-500">Hero background image URL</label>
                  <input value={current.heroImage}
                    onChange={(e) => { setCurrentField("heroImage", e.target.value); }}
                    placeholder="https://…" className={inputCls + " font-mono text-slate-400"} />
                </div>
                <div>
                  <label className="block mb-1 text-xs font-medium uppercase tracking-wider text-slate-500">CTA button text</label>
                  <input value={current.heroCtaText}
                    onChange={(e) => { setCurrentField("heroCtaText", e.target.value); }}
                    placeholder="Get started" className={inputCls} />
                </div>
                <div>
                  <label className="block mb-1 text-xs font-medium uppercase tracking-wider text-slate-500">CTA button URL</label>
                  <input value={current.heroCtaUrl}
                    onChange={(e) => { setCurrentField("heroCtaUrl", e.target.value); }}
                    placeholder="/blog" className={inputCls + " font-mono text-slate-400"} />
                </div>
              </div>
            </Collapsible>
          </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-4">
                <div>
                  ...
                  <input value={current.metaTitle}
                    onChange={(e) => setCurrentField("metaTitle", e.target.value.slice(0, 255))}
                    placeholder={current.title || "Page title"} className={inputCls} />
                </div>
                <div>
                  <label className="block mb-1 text-xs font-medium uppercase tracking-wider text-slate-500">
                    Meta description <span className="text-slate-600">({current.metaDescription.length}/500)</span>
                  </label>
                  <textarea value={current.metaDescription}
                    onChange={(e) => setCurrentField("metaDescription", e.target.value.slice(0, 500))}
                    rows={3} className={inputCls + " resize-none"} />
                </div>
                <div>
                  <label className="block mb-1 text-xs font-medium uppercase tracking-wider text-slate-500">OG Image URL</label>
                  <input value={current.ogImage}
                    onChange={(e) => setCurrentField("ogImage", e.target.value)}
                    placeholder="https://…" className={inputCls + " font-mono text-slate-400"} />
                </div>
              </div>
              <div>
                <p className="mb-2 text-xs font-medium uppercase tracking-wider text-slate-500">SERP Preview</p>
                <div className="rounded-lg border border-slate-700 bg-slate-950 p-4 space-y-1">
                  <div className="text-xs text-green-600 font-mono truncate">example.com/{activeLocale}/{serpSlug}</div>
      ...
      <div className={["gap-6", viewMode === "split" ? "grid md:grid-cols-2" : "flex flex-col"].join(" ")}>
        {(viewMode === "editor" || viewMode === "split") && (
          <div className="rounded-xl border border-slate-800 bg-slate-900/60 overflow-hidden flex flex-col">
            <RichTextEditor key={activeLocale} content={current.content} onChange={handleContentChange}
              placeholder="Start writing…" className="min-h-[60vh]" />
          </div>
        )}
        {(viewMode === "preview" || viewMode === "split") && (
          <div className="rounded-xl border border-slate-800 bg-slate-900/40 overflow-hidden">
            <p className="px-6 pt-5 pb-3 text-xs font-medium uppercase tracking-wider text-slate-600">Preview</p>
            ...
            {pageConfig.showHero && (
              ...
              >
                {current.title && <h1 className="text-3xl font-bold text-white leading-tight">{current.title}</h1>}
                {current.heroSubtitle && <p className="text-slate-300 text-lg">{current.heroSubtitle}</p>}
                {current.heroCtaText && (
                  <a href={current.heroCtaUrl || "#"}
            ...
            <div className="px-6 pb-6">
              ...
              {pageConfig.showToc && (
                <TableOfContents doc={current.content} className="mb-6" />
              )}
