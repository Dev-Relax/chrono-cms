...
export const webhooksRoutes = async (fastify: FastifyInstance): Promise<void> => {
  fastify.get("/admin/webhooks", { preHandler: [fastify.authenticate] }, async (request, reply) => {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const hook = await prisma.webhook.create({
        data: {
          ...createRest,
          ...(createSecret !== undefined && { secret: createSecret }),
        },
      })
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const body = JSON.stringify({
        ...
        data: { message: "Test ping from Chronos CMS" },
      })
      ...
      try {
        ...
        return reply.status(502).send({
          error: `Could not reach ${hook.url}: ${(err as Error).message}`,
        })
      }
    },
  )
