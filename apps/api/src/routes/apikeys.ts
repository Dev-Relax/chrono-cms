...
export const apiKeysRoutes = async (fastify: FastifyInstance): Promise<void> => {
  fastify.get("/admin/apikeys", { preHandler: [fastify.authenticate] }, async (request, reply) => {
  ...
  fastify.post("/admin/apikeys", { preHandler: [fastify.authenticate] }, async (request, reply) => {
    if (!(await requireRole(request, reply, "ADMIN"))) return
