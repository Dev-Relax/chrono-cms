...
export const mediaRoutes = async (fastify: FastifyInstance): Promise<void> => {
  fastify.post("/admin/media", { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    if (RASTER_MIMES.has(data.mimetype)) {
      ...
      finalBuffer = buffer
      const originalExt = extname(data.filename ?? "")
        .toLowerCase()
        .replace(".", "")
  ...
  fastify.get("/admin/media", { preHandler: [fastify.authenticate] }, async (_request, reply) => {
