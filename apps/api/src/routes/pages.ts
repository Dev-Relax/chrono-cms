...
type PageWithTranslations = Prisma.PageGetPayload<{
  select: typeof pageWithTranslationsSelect
}>
...
const flattenPage = (page: PageWithTranslations, locale: string, format: ContentFormat) => {
  ...
  const hreflang = page.translations.map((t) => ({
    locale: t.locale,
    slug: t.slug,
  }))
...
export const pagesRoutes = async (fastify: FastifyInstance): Promise<void> => {
  ...
  fastify.get("/pages", async (request, reply) => {
    ...
    const data = pages.map((p) => flattenPage(p, lang ?? p.defaultLocale, format)).filter(Boolean)
  ...
  fastify.get("/admin/pages", { preHandler: [fastify.authenticate] }, async (request, reply) => {
  ...
  fastify.post("/admin/pages", { preHandler: [fastify.authenticate] }, async (request, reply) => {
    if (!(await requireRole(request, reply, "EDITOR"))) return
      ...
      data: {
        ...
        translations: {
          create: Object.entries(translations).map(([locale, tr]) => ({
            ...
            ...(tr.metaDescription !== undefined && {
              metaDescription: tr.metaDescription,
            }),
            ...(tr.ogImage !== undefined && { ogImage: tr.ogImage }),
            ...(tr.heroContent !== undefined && {
              heroContent: tr.heroContent as Prisma.InputJsonValue,
            }),
          })),
        },
      },
    ...
    const webhookTranslations = page.translations.map((t) => ({
      locale: t.locale,
      title: t.title,
      slug: t.slug,
    }))
