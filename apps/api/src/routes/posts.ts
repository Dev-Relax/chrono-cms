...
type PostWithTranslations = Prisma.PostGetPayload<{
  select: typeof postWithTranslationsSelect
}>
...
const flattenPost = (
  ...
  const hreflang = post.translations.map((t) => ({
    locale: t.locale,
    slug: t.slug,
  }))
...
const saveRevision = async (
  ...
  if (all.length > 10) {
    await prisma.postRevision.deleteMany({
      where: { id: { in: all.slice(10).map((r) => r.id) } },
    })
  }
}
...
export const postsRoutes = async (fastify: FastifyInstance): Promise<void> => {
  ...
  fastify.get("/posts", async (request, reply) => {
    ...
    const where = {
      ...
      ...(strict && query.lang
        ? { translations: { some: { locale: query.lang.trim().slice(0, 10) } } }
        : {}),
    }
  ...
  fastify.get("/admin/posts", { preHandler: [fastify.authenticate] }, async (request, reply) => {
    const query = request.query as {
      page?: string
      limit?: string
      status?: string
    ...
    async (request, reply) => {
      ...
      if (action === "delete") {
        ...
        logActivity({
          ...
          action: "post.deleted",
          entityType: "post",
          entityTitle: `${affected} posts (bulk)`,
        })
      } else {
  ...
  fastify.post("/admin/posts", { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    const post = await prisma.post.create({
      data: {
        ...
        translations: {
          create: Object.entries(translationData).map(([locale, t]) => ({
            locale,
            ...t,
          })),
        },
    ...
    const webhookTranslations = post.translations.map((t) => ({
      locale: t.locale,
      title: t.title,
      slug: t.slug,
    }))
  ...
  fastify.put(
    ...
    async (request, reply) => {
      ...
      if (!isOwnerOrMinRole(request, existing.authorId))
        return reply.status(403).send({ error: "Insufficient permissions" })

      const payload = request.user as { sub: string }
      const resolvedDefault = body.data.defaultLocale ?? existing.defaultLocale
      const translationData = buildTranslations(
        body.data as z.infer<typeof postBodySchema>,
        resolvedDefault,
      )
      ...
      await Promise.all(
        Object.keys(translationData).flatMap((locale) => {
          ...
          return current
            ? [saveRevision(id, locale, current.title, current.content, payload.sub)]
            : []
        }),
      )
      ...
      const webhookTranslations = updated.translations.map((t) => ({
        locale: t.locale,
        title: t.title,
        slug: t.slug,
      }))
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      logActivity({
        ...
        entityType: "post",
        entityId: id,
        entityTitle: `Restored: ${revision.title}`,
      })
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const token = fastify.jwt.sign({ previewId: id, purpose: "preview" }, { expiresIn: "1h" })
