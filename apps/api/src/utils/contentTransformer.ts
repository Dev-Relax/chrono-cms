  ...
  switch (node.type) {
    ...
    case "image": {
      ...
      const style = (() => {
        ...
        switch (align) {
          case "left":
            return `float:left;width:${w ?? "auto"};max-width:60%;margin-right:1.5rem;margin-bottom:0.5rem`
          case "right":
            return `float:right;width:${w ?? "auto"};max-width:60%;margin-left:1.5rem;margin-bottom:0.5rem`
          case "center":
            return `display:block;width:${w ?? "auto"};max-width:100%;margin-left:auto;margin-right:auto`
          default:
            return `display:block;width:${w ?? "100%"};max-width:100%`
        }
      })()
...
const nodeToMd = (node: PmNode, listDepth = 0, ordered = false, index = 0): string => {
  switch (node.type) {
    ...
    case "bulletList":
      return (node.content ?? []).map((n) => nodeToMd(n, listDepth + 1, false)).join("\n")

    case "orderedList":
      return (node.content ?? []).map((n, i) => nodeToMd(n, listDepth + 1, true, i + 1)).join("\n")

    case "listItem": {
      const indent = "  ".repeat(listDepth - 1)
      const bullet = ordered ? `${index}.` : "-"
      const inner = (node.content ?? [])
        .map((n) => nodeToMd(n, listDepth, ordered, index))
        .join("\n")
